...
class Ranked:
    ...
    snippet: str = ""
    depth_hits: list = field(default_factory=list)
    location: str = ""
...
def rank_candidates(path, jd: JobSpec = REDROB_SENIOR_AI_ENGINEER, limit=None):
    t0 = time.time()
    cards: list[Ranked] = []
    n = n_hp = 0
    for c in stream_candidates(path, limit=limit):
        ...
        cards.append(Ranked(
            ...
            snippet=_clean_snippet(r.best_role_desc), depth_hits=r.depth_hits,
    ...
    stats = {"total": n, "honeypots_excluded": n_hp, "scored": len(cards),
